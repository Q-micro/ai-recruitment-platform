...
export default function CandidateJobs() {
  ...
  async function fetchPageData() {
    try {
      ...
      if (candidateId) {
        ...
        const [profile, generatedCvs] = await Promise.all([
          apiFetch(`/api/candidate/profile/${candidateId}`),
          apiFetch(`/api/candidate/generated-cvs/${candidateId}`),
        ]);

        const options = [
          profile?.cv_url && {
            label: "Original uploaded CV",
            url: profile.cv_url,
          },
          ...(Array.isArray(generatedCvs) ? generatedCvs : []).map((cv) => ({
            label: cv.title || `ATS CV #${cv.id}`,
            url: cv.cv_url,
          })),
        ].filter((cv) => cv && cv.url);
        ...
        if (options.length > 0) {
          setSelectedCvUrl(options[0].url);
        }
      }
    } catch (error) {
      console.error(error);
  ...
  function handleApply(jobId) {
    ...
    if (cvOptions.length === 1) {
      setSelectedCvUrl(cvOptions[0].url);
    }
  ...
  async function handleConfirmApply() {
    try {
      ...
      if (!selectedCvUrl) {
        setErr("Please choose which CV you want to submit.");
        return;
      }

      if (appliedJobIds.includes(pendingApplyJobId)) return;

      setApplyingJobId(pendingApplyJobId);

      await apiFetch("/api/candidate/apply", {
        method: "POST",
        body: JSON.stringify({
          candidate_id: candidateId,
          job_id: pendingApplyJobId,
          cover_letter: null,
      ...
      console.error(error);
      setErr(error?.response?.data?.message || error?.message || "Failed to apply");
    } finally {
      setApplyingJobId(null);
    }
  }
          ...
          <>
            ...
            >
              ...
              <DialogContent dividers sx={{ borderColor: colors.cardBorder }}>
                <Stack spacing={2}>
                  <Typography sx={{ color: colors.subtext, fontSize: 14 }}>
                    Select which CV you want to send with this application.
                  </Typography>
                  ...
                  {cvOptions.length > 0 ? (
                    ...
                    >
                      {cvOptions.map((cv) => (
                        <MenuItem key={cv.url} value={cv.url}>
                          {cv.label}
                        </MenuItem>
                      ))}
                    </TextField>
                    ...
                    <Alert severity="warning" sx={{ borderRadius: 3 }}>
                      No CV found. Please upload or generate a CV before applying.
                    </Alert>
                  )}
                </Stack>
              </DialogContent>
              ...
              >
                ...
                <Button
                  ...
                  disabled={!selectedCvUrl || !pendingApplyJobId || Boolean(applyingJobId)}
                  sx={actionButtonSx(colors, "contained")}
                >
